      ...
      .then(res => {
        if (res.ok) {
          ...
          Alert.alert('无法进入YeChat', '检查更新失败', [
            {text: '退出', onPress: () =>{ navigation.navigate('Home');BackHandler.exitApp();} },
          ]);
        }
      })
      .catch(err => {
    ...
    <ImageBackground
      ...
      <Animatable.View animation="fadeIn" style={{flex: 1}} delay={500}>
        ...
        <Animatable.View
          ...
          duration={1000}>
          <TouchableOpacity
            onPress={() => { checkUpdate();navigation.navigate('Login') }}
            style={[
              styles.button2,
              {
          ...
          <TouchableOpacity
            onPress={() => { checkUpdate(); navigation.navigate('Signup') }}
            style={[
              styles.button2,
              useColorScheme() === 'dark'
